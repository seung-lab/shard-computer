...
uint64_t MurmurHash3_x86_64(const uint64_t key, uint32_t seed) {
  const uint64_t len = sizeof(uint64_t);
  ...
  // omit switch statement since always 8
  k2 ^= ((key >> 56) & 0xff) << 24; // case 8
  k2 ^= ((key >> 48) & 0xff) << 16; // case 7 
  k2 ^= ((key >> 40) & 0xff) << 8;  // case 6 
  k2 ^= ((key >> 32) & 0xff) << 0;  // case 5
    k2 *= c2; k2  = ROTL32(k2,16); k2 *= c3; h2 ^= k2;

  k1 ^= ((key >> 24) & 0xff) << 24; // case 4
  k1 ^= ((key >> 16) & 0xff) << 16; // case 3
  k1 ^= ((key >> 8) & 0xff) << 8;  // case 2
  k1 ^= ((key >> 0) & 0xff) << 0; // case 1
    k1 *= c1; k1  = ROTL32(k1,15); k1 *= c2; h1 ^= k1;
